...
function luckyThree(){
    ...
    function cloneGrid(grid) {
        ...
        // const newGrid = grid.map(u => Object.assign({}, u));
        // return newGrid
        ...
        // const deepCopy = (grid) => {
            ...
            //   if(Array.isArray(elem)){
                // newGrid.push(deepCopy(elem))
            //   }else{
                // if (typeof elem === 'object') {
                //   newCopy.push(deepCopyObject(elem))
            //   } else {
                //   newCopy.push(elem)
                // }
            //   }
            ...
            for (var i = 0; i < width; i++) {
                ...
                for (var j = 0; j < height; j++) {
                    const square = document.createElement('img')
                // square.src = balloons[randomColor]
                    square.setAttribute('src',grid[i][j].getAttribute('src'))
                    square.setAttribute('id', grid[i][j].getAttribute('id'))
                    newCopy[i][j] = square ; 
    ...
    var gridForCheckingRows = new Array(); 
    var gridForCheckingCols = new Array(); 

    var fromCheckedRows; 
    var fromCheckedCols; 
    
    function compareEmptySrc(){
        for (var i = 0; i < width; i++) {
            for (var j = 0; j < height; j++) {
                fromCheckedRows = gridForCheckingRows[i][j].getAttribute('src')
                fromCheckedCols = gridForCheckingCols[i][j].getAttribute('src')
                var fromCheckedRowsID = gridForCheckingRows[i][j].getAttribute('id')
                var fromCheckedColsID = gridForCheckingCols[i][j].getAttribute('id')
                if((fromCheckedRows == "") || (fromCheckedCols == "")){
                    gfg[i][j].setAttribute('src', "")
                    
                console.log('row src atr and id'+ fromCheckedRows + fromCheckedRowsID)
                console.log('col src atr and id'+ fromCheckedCols + fromCheckedColsID)
                console.log('gfg src atr and id'+ gfg[i][j].getAttribute('src'))
                }
            }
        }
        fillTheEmptySrc()
    ...
    function swapElements(){
        ...
        if((swapElementSrc1 == 'hedgehog.png') || (swapElementSrc2 == 'hedgehod.png')){
            ...
            gridForCheckingRows = cloneGrid(gfg)
            gridForCheckingCols = cloneGrid(gfg)
            threeOrMoreInARow();

            threeOrMoreInAColumn();
            // console.log(gridForCheckingRows)
...
function threeOrMoreInARow(){
    for (i = 0; i < gridForCheckingRows.length; i++) {
        for (j = 0; j < gridForCheckingRows[i].length-1 ; j++) {
             
            tempBalloon = gridForCheckingRows[i][j].getAttribute('src') 
            nextBalloon = gridForCheckingRows[i][j+1].getAttribute('src') 
            ...
            if (((tempBalloon != nextBalloon) && (balloonCountInARow>2)) || ((j==7) && (balloonCountInARow>2))) {
                ...
                for ( k=0;k<=i; k++){
                    for (y=0; y<balloonCountInARow; y++){
                        ...
                        if ((i-k)>=1){
                            balloonColor = gridForCheckingRows[i-k-1][j-y].getAttribute('src')
                            gridForCheckingRows[i-k][j-y].setAttribute('src',balloonColor)
                            }

                        if ((i-k)==0){
                            gridForCheckingRows[i-k][j-y].setAttribute('src','')
                        }
                    }
                }
...
function threeOrMoreInAColumn(){
    for ( j = 0; j < 9 ; j++) {
        ...
        tempBalloon = gridForCheckingCols[i][j].getAttribute('src') 
        nextBalloon = gridForCheckingCols[i+1][j].getAttribute('src') 
            ...
            for (var t=0; t<=i; t++){
                ...
                if((i-t)>(balloonCountInAColumn-1)){
                    ...
                    elementForumula = i - t - balloonCountInAColumn
                        balloonColor = gridForCheckingCols[elementForumula][j].getAttribute('src')
                        gridForCheckingCols[i-t][j].setAttribute('src',balloonColor)
                        gridForCheckingCols[elementForumula][j].setAttribute('src','')                    
                }
                if((i-t) <= (balloonCountInAColumn-1)) { 
                    i = parseInt(i)
                    t = parseInt(t)
                    gridForCheckingCols[i-t][j].setAttribute('src',"")
                }
            }
